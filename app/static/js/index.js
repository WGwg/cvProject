$(document).ready(function () {
    $("#fileimg").change(function (e) {
        ...
        $("#txturl").val(file.name);

        // if (validate_img(file))
        //     submit($("#imgform"));
    });

     $("#imageRetrieval").click(function () {
        var file = $("#fileimg")[0].files[0];
        if (validate_img(file))
            submit($("#imgform"));
    });

      $("#edgeDetction").click(function () {
        var file = $("#fileimg")[0].files[0];
        if (validate_img(file))
            submit($("#imgform"));
    });

      $("#hough").click(function () {
        var file = $("#fileimg")[0].files[0];
        if (validate_img(file))
            submit($("#imgform"));
    });

      $("#denoising").click(function () {
        var file = $("#fileimg")[0].files[0];
        if (validate_img(file))
            submit($("#imgform"));
    });
     $("#trafficSignCode").click(function () {
            var file = $("#fileimg")[0].files[0];
            if (validate_img(file))
                submit($("#imgform"));
        });
     ...
     $("#Histogram_equalization").click(function () {
                    var file = $("#fileimg")[0].files[0];
                    if (validate_img(file))
                        submit($("#imgform"));
                });
     $("#sift_extract").click(function () {
                    var file = $("#fileimg")[0].files[0];
                    if (validate_img(file))
                        submit($("#imgform"));
                });
